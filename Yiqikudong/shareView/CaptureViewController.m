...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"CaptureView" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(removeCaptureView) name:@"CaptureView" object:nil];
    [[NSNotificationCenter defaultCenter]removeObserver:self name:@"startCapture" object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(startCapture) name:@"startCapture" object:nil];
}
...
-(void)startCapture
{
    if (![_recorder.captureSession isRunning])
    {
...
- (void)initRecorder
{
    ...
    _recorder.preViewLayer.frame = CGRectMake(0, 0, DEVICE_SIZE.width, 300);
...
- (void)initProgressBar
{
    ...
    [SBCaptureToolKit setView:_progressBar toOriginY:300];
...
{
    CGFloat buttonW = 70.0f;
    self.recordButton = [[UIButton alloc] initWithFrame:CGRectMake((DEVICE_SIZE.width - buttonW) / 2.0, _progressBar.frame.origin.y + _progressBar.frame.size.height , buttonW, buttonW)];
...
//        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提醒" message:@"确定放弃这个视频吗?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"放弃", nil];
...
- (void)dropTheVideo
{
    ...
    NSNotification *notification = [NSNotification notificationWithName:@"videoView" object:nil];
...
- (void)videoRecorder:(SBVideoRecorder *)videoRecorder didFinishMergingVideosToOutPutFileAtURL:(NSURL *)outputFileURL
{
    ...
    NSDictionary*dict = [NSDictionary dictionaryWithObject:outputFileURL forKey:@"videourl"];
    NSNotification *notification = [NSNotification notificationWithName:@"playvideoView" object:self userInfo:dict];
    [[NSNotificationCenter defaultCenter]postNotification:notification];
}
