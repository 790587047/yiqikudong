...
-(void)drawRect:(CGRect)rect
{
    ...
    if(line>0){
        ...
        CGSize fontSize = [self.text boundingRectWithSize:self.bounds.size options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
        
//        CGSize fontSize =[self.text sizeWithFont:self.font
//                                        forWidth:self.bounds.size.width
//                                   lineBreakMode:UILineBreakModeTailTruncation];
        // Get the fonts color.
        const float * colors = (const float*)CGColorGetComponents(self.textColor.CGColor);
